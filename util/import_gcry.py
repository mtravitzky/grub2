...
for cipher_file in cipher_files:
    ...
    if re.match ("(Manifest|Makefile\.am|ac\.c|cipher\.c|hmac-tests\.c|pubkey\.c|gostr3411-94.c|stribog.c)$", cipher_file) or cipher_file == "kdf.c" or cipher_file == "elgamal.c" or cipher_file == "primegen.c" or cipher_file == "ecc.c" or cipher_file == "test-getrusage.c" or cipher_file == "salsa20.c":
        chlog = "%s%s: Removed\n" % (chlog, chlognew)
        continue
    # Autogenerated files. Not even worth mentionning in ChangeLog
    if re.match ("Makefile\.in$", cipher_file):
        continue
    if re.match ("sha256-ssse3-amd64.S$", cipher_file):
        f = codecs.open (infile, "r", "utf-8")
        fw = codecs.open (outfile, "w", "utf-8")
        fw.write ("/* This file was automatically imported with\n")
        fw.write ("   import_gcry.py. Please don't modify it */\n")
        fw.write ("#define HAVE_COMPATIBLE_GCC_AMD64_PLATFORM_AS 1\n")
        fw.write ("#define HAVE_INTEL_SYNTAX_PLATFORM_AS 1\n")
        fw.write ("#define HAVE_GCC_INLINE_ASM_SSSE3 1\n")
        fw.write ("#define USE_SHA256 1\n")
        fw.write (f.read ())
        f.close ()
        fw.close ()
        confutil.write ("  common = %s;\n" % outfile)
        continue
    nch = False
    if re.match (".*\.[ch]$", cipher_file):
        ...
        for line in f:
            ...
            m = re.match ("#undef USE_SSSE3", line)
            if cipher_file == "sha256.c" and not m is None:
                fw.write ("#define USE_SSSE3 1\n")
                continue
            ...
            if isc and not m is None:
                ...
                continue
            fw.write (line)
        if len (ciphernames) > 0 or len (mdnames) > 0 or len (pknames) > 0 or cipher_file == "hash-common.c":
            if isglue:
            ...
            for src in modfiles.split():
                conf.write ("  common = %s;\n" % src)
                if len (ciphernames) > 0 or len (mdnames) > 0 or cipher_file == "hash-common.c":
                    confutil.write ("  common = grub-core/%s;\n" % src)
            if modname == "gcry_sha256":
                conf.write ("  common = lib/libgcrypt-grub/cipher/sha256-ssse3-amd64.S;\n")
            if modname == "gcry_ecc":
