		  ...
		  struct grub_linux_initrd_context *initrd_ctx)
{
  ...
  name_len = grub_strlen (comp->newc_name);
		  ...
		  struct grub_linux_initrd_context *initrd_ctx)
{
  ...
  FOR_LIST_ELEMENTS (pk, kpuber)
    if (pk->key && pk->path)
      grub_initrd_component (pk->key, grub_strlen (pk->key), pk->path, initrd_ctx);
  return GRUB_ERR_NONE;
...
grub_err_t
grub_initrd_publish_key (const char *uuid, const char *key, const char *path)
{
